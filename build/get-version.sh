...
major="`sed -n '/#define.*SERF_MAJOR_VERSION/s/^.*\([0-9][0-9]*\).*$/\1/p' $2`"
minor="`sed -n '/#define.*SERF_MINOR_VERSION/s/^.*\([0-9][0-9]*\).*$/\1/p' $2`"
patch="`sed -n '/#define.*SERF_PATCH_VERSION/s/^.*\([0-9][0-9]*\).*$/\1/p' $2`"
